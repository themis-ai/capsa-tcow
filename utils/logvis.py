...
class MyLogger(logvisgen.Logger):
    ...
    def handle_train_step_mask_track(self, epoch, phase, cur_step, total_step, steps_per_epoch,
        ...
        # (Qs, 3, T, H, W).
        if train_args.wrapper != "none" and 'train' not in phase: output_mask_risk = model_retval['output_mask_risk'][0].detach().cpu().numpy()
        # (Qs, 3, T, H, W).
        ...
        if source_name == 'plugin':
            ...
            if train_args.wrapper != "none" and 'train' not in phase: output_mask_risk = output_mask_risk[None]
